...
void AddDrone_6()
{
	...
	char *libname = "..\\drone_asd_error7_.dll";
	...
	FILE *file = fopen(File7, "wb+");
	while (time <= SimulationTime)
	{
		WaitForSingleObject(g_hThreadEvent[7], INFINITE);
			for (i = 0; i < numinputs_to_model; i++)
			{
				if (i == 0)
				{
