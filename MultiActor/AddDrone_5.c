...
void AddDrone_5()
{
	...
	char *libname = "..\\drone_asd_error6_.dll";
	...
	FILE *file = fopen(File6, "wb+");
	while (time <= SimulationTime)
	{
		WaitForSingleObject(g_hThreadEvent[6], INFINITE);
			for (i = 0; i < numinputs_to_model; i++)
			{
				if (i == 0)
				{
