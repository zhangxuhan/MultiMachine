...
void AddDrone_8()
{
	...
	char *libname = "..\\drone_asd_error9_.dll";
	...
	FILE *file = fopen(File9, "wb+");
	while (time <= SimulationTime)
	{
		WaitForSingleObject(g_hThreadEvent[9], INFINITE);
			for (i = 0; i < numinputs_to_model; i++)
			{
				if (i == 0)
				{
